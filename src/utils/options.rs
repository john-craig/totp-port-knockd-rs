...
pub fn get_config(config_path: &Path) -> Result<TOTPKnockConfig, Box<dyn std::error::Error>> {
    ...
    let config: TOTPKnockConfig = if config_path.exists() {
        ...
        let config_outer: KnockConfig = toml::from_str(str::from_utf8(&config_contents)?)?;
...
impl KnockCommon {
    pub fn build_common(
        state_dir: &Path,
        args: KnockArgs,
        config: TOTPKnockConfig,
    ) -> Result<KnockCommon, Box<dyn std::error::Error>> {
            ...
            Some(asp) => {
                match args.secret_value {
                    Some(_) => {
                        ...
                        // and a secret value
                        log::error!(
                            "Command line options included both '--secret-path'\
        ...
        let conf_secret_value: Option<String> = (match config.secret_path {
            ...
            None => match config.secret_value {
                Some(csv) => {
    ...
    pub fn interval_remaining(&self) -> Result<u64, Box<dyn std::error::Error>> {
        ...
        let interval_elapsed = cur_time_secs % self.interval;
        let interval_remaining = if interval_elapsed == 0 {
            self.interval
        } else {
            self.interval - interval_elapsed
        };
